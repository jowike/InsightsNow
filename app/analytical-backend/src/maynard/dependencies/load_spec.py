...
def load_spec(specfile: str) -> Dict[str, list]:
    """
    Load model specification from an Excel file.

    The Excel file should contain a structured specification table with the following
    columns (case-insensitive): Model, SeriesID, SeriesName, Frequency, Transformation,
    Units, Category, and Region. 

    The function filters rows where 'Model' ≠ 0, extracts metadata
    and returns a dictionary suitable for use in modeling.

    Args:
        specfile: path to the Excel file containing the model specification.

    Returns:
        spec: parsed specification as a dictionary with keys such as
              'seriesid', 'seriesname', 'transformation', etc.
    """
    ...
    spec["blocknames"] = [
        col.replace("block", "").replace("-", "") for col in block_cols
    ]
