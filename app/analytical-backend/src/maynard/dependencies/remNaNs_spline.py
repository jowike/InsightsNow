...
def filter(x: Union[np.ndarray, list[float]], k: int) -> np.ndarray:
    """
    Apply a moving average filter with a window of 2*k+1.

    Args:
        x: Input signal or time series as a list or NumPy array.
        k: Half window size. The full window will be of size 2k + 1.
